...
数据库表由 Flyway 管理，当前版本为 `V11`。第十三周新增转发、治理关系、推荐反馈、趋势快照、任务审计、消费幂等、人工处理和审计归档表。V1～V11 是不可修改的历史迁移，后续结构变化必须新增 V12。当前 34 张业务表的完整字段、主外键、索引、ER 图和 Redis/MinIO/RabbitMQ/OpenSearch 数据边界见[数据库文档](docs/数据库文档.md)。
