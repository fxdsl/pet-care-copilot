...
1. 已执行的 Flyway 历史迁移 `V1～V11` 不得修改、删除或重新编号。
2. 表、字段、索引、外键或检查约束发生变化时，新增下一个 Flyway 迁移，例如下一版使用 `V12__describe_change.sql`。
...
下一次结构变更必须从 `V12` 开始，不得修改上表中的历史 SQL。
