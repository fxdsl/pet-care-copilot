...
1. 已执行的 Flyway 历史迁移 `V1～V10` 不得修改、删除或重新编号。
2. 表、字段、索引、外键或检查约束发生变化时，新增下一个 Flyway 迁移，例如下一版使用 `V11__describe_change.sql`。
...
6. `flyway_schema_history` 是 Flyway 自动维护的系统表，不计入 26 张业务表。
...
下一次结构变更必须从 `V11` 开始，不得修改上表中的历史 SQL。
